...
export const sendRegistrationEmail = async (userDetails) => {
  const {
    email, name, email_verification_otp,
  } = userDetails;
...
export const sendForgotPasswordEmail = async (userDetails) => {
  const {
    email, name, forgot_password_otp,
  } = userDetails;
...
export const sendChangePasswordEmail = async (userDetails) => {
  const {
    email, name,
  } = userDetails;
  ...
  const emailDetails = {
    to: email,
    subject: emailMessages.CHANGE_PASSWORD,
    template: path.join(__dirname, '../views/emailTemplates/changePassword.hbs'),
    templateData: {
      name,
      email,
      url: process.env.WEB_APP_BASE_URL,
